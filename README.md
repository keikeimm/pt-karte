...
| 頭に顧客データ | クライアント詳細は顧客データを常時上部表示。エディタ上部にも顧客サマリーを常時表示（タブとしては持たせず重複を避けている） |
| カルテは記入日で管理 | 「＋ カルテを作成」で記入日を選んで作成（自動で本日にはしない）。同じ日付は重複作成できない。記入日はその場で変更可、一覧は日付の新しい順 |
...
| 明示的な保存ボタン | カウンセリングシート・注意書き・カルテのいずれも「保存」ボタンを押すまでは確定しない |
| 未保存の変更を確認 | 保存せずに画面を離れようとすると確認ダイアログが出る。キャンセルすれば編集内容を保持したまま留まれる |
